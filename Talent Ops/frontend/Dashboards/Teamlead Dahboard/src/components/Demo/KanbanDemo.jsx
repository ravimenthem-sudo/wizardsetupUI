...
const KanbanDemo = () => {
    ...
    // Filter State
    const [selectedTeam, setSelectedTeam] = useState(currentTeam || 'All');
    const [selectedEmployee, setSelectedEmployee] = useState('All');

    // Sync with global team setting and navigation state
    React.useEffect(() => {
        ...
        if (location.state?.employeeName) {
            // If team is provided, set it (unless global overrides)
            if (location.state.team && (!currentTeam || currentTeam === 'All')) {
                setSelectedTeam(location.state.team);
            }
            setSelectedEmployee(location.state.employeeName);
        }
