...
const InvoiceGenerator = () => {
  ...
  const generateInvoice = async () => {
    try {
      ...
      setShowPreview(true);
      alert(`✅ Invoice ${invoiceData.invoiceNumber} saved to database!\nPreview shown below.`);
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      if (!serviceId || !templateId || !publicKey) {
        ...
        alert('📧 Email client opened!\n\nNote: Add EmailJS credentials in .env for automatic sending.');
        setShowShareModal(false);
        setLoading(false);
        return;
      }
      ...
      alert('✅ Email sent successfully!\n📧 Invoice delivered to: ' + emailData.to);
      setShowShareModal(false);
    ...
    <div className="invoice-generator">
      <header className="invoice-header">
        ...
        <div className="actions">
          <button className="btn primary" onClick={generateInvoice} disabled={loading}>
            {loading ? 'Saving...' : 'Generate Invoice'}
          </button>
          <button className="btn secondary" onClick={generatePDF}>Preview PDF</button>
