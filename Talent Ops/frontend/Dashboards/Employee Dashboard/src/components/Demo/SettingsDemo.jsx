...
import { Bell, Lock, User, Globe, Moon, Save, Users } from 'lucide-react';
import { useToast } from '../../context/ToastContext';
import { useUser } from '../../context/UserContext';

const SettingsDemo = () => {
    const { addToast } = useToast();
    const { userName, setUserName, currentTeam, setCurrentTeam } = useUser();
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-xl)', maxWidth: '800px' }}>
            ...
            <section style={{ backgroundColor: 'var(--surface)', padding: 'var(--spacing-lg)', borderRadius: '16px', boxShadow: 'var(--shadow-sm)' }}>
                <h3 style={{ fontSize: '1.1rem', fontWeight: 'bold', marginBottom: 'var(--spacing-lg)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <Users size={20} /> Team Preferences
                </h3>
                <div>
                    <label style={{ display: 'block', fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '8px' }}>Active Team View</label>
                    <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '12px' }}>
                        Select a team to filter the entire dashboard view. Select 'All' to see an overview of all teams.
                    </p>
                    <select
                        value={currentTeam}
