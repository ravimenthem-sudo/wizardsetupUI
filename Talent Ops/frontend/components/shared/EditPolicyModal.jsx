...
export const EditPolicyModal = ({ isOpen, onClose, onSuccess, policy }) => {
        ...
        const fetchCategories = async () => {
            ...
            try {
                ...
                if (data && data.length > 0) {
                    ...
                    // Combine and sort
                    const allCategories = [...new Set([...currentDefaults, ...usedCategories])].sort();
    ...
    useEffect(() => {
        if (policy) {
            const category = policy.category || 'General Policy';
            const isStandard = categories.includes(category);

            setFormData(prev => ({
                ...prev,
                title: policy.title || policy.name || '',
                category: isStandard ? category : 'Other',
            ...
            if (policy !== prevPolicyRef.current) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            let fileUrl = policy.file_url; // Keep existing file URL by default

            // If user uploaded a new file, upload it
            if (policyFile && !keepExistingFile) {
                ...
                console.log('New file uploaded. Public URL:', publicUrl);
                fileUrl = publicUrl;

                // Delete old file if it exists
                if (policy.file_url) {
                    try {
                        const oldFilePath = policy.file_url.split('/policies/')[1];
                        if (oldFilePath) {
            ...
            // Update policy record in database
            ...
            if (dbError) {
                ...
                throw new Error(`Failed to update policy: ${dbError.message}`);
            }
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold', margin: 0 }}>Edit Policy</h2>
                    <button
                        onClick={handleClose}
                ...
                <form onSubmit={handleSubmit} style={{ padding: '24px' }}>
                    ...
                    <div style={{ marginBottom: '24px' }}>
                        <label style={{
                            ...
                            Policy Document (PDF) - Optional
                        </label>
                        <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: '8px' }}>
                            {keepExistingFile && !policyFile ? 'Current file will be kept if you don\'t upload a new one' : 'Upload a new file to replace the existing one'}
                        </p>
                        <div style={{
                            ...
                            <input
                                ...
                                disabled={isUploading}
                                style={{ display: 'none' }}
                                id="policy-file-upload-edit"
                            />
                            <label
                                htmlFor="policy-file-upload-edit"
                                style={{
                                    cursor: isUploading ? 'not-allowed' : 'pointer',
                                    display: 'flex',
                    ...
                    <div style={{
                        ...
                        <button
                            ...
                            style={{
                                ...
                                borderRadius: '8px',
                                border: 'none',
                                backgroundColor: isUploading ? '#ccc' : '#f59e0b',
                                color: 'white',
                                cursor: isUploading ? 'not-allowed' : 'pointer',
                                fontWeight: 600
                            }}
                        >
                            {isUploading ? 'Updating...' : 'Update Policy'}
                        </button>
                    </div>
                </form>
            </div>
